...
# TODO: Crie um método fazer_chamada para permitir que um usuário faça uma chamada telefônica:
    def fazer_chamada(self, destinatario, duracao):
        custo = self.plano.custo_chamada(duracao=duracao)
        saldo = self.plano.verificar_saldo()

        if custo <= saldo:
            deducao = self.plano.deduzir_saldo(custo)

            return f'Chamada para {destinatario} realizada com sucesso. Saldo: ${deducao:.2f}'
        else:
